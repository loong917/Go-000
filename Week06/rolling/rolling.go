...
// Number tracks a numberBucket over a bounded number of time buckets.
...
func NewNumber(threshold int64) *Number {
	r := &Number{
		Threshold: threshold,
...
func (r *Number) Sum(now time.Time) float64 {
	...
	for timestamp, bucket := range r.Buckets {
		if timestamp >= now.Unix()-r.Threshold {
			sum += bucket.Value
		}
	}
...
// Avg returns the average value seen in the last 10 seconds.
func (r *Number) Avg(now time.Time) float64 {
	return r.Sum(now) / float64(r.Threshold)
}
